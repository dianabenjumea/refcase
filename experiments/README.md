...
This directory contains the experiment runner and metrics post-processing tools for the SRAS state machine in `refcase_ws` and the SS Gwendolen MCAPL agent.
...
- the ROS/Gazebo environment via `refcase_ws/launch.sh`
- the SMACH state machine via `refcase_ws/launch_smach.sh`
...
- ROS Melodic with the `refcase_ws` workspace built and sourced by its launch scripts
...
The script assumes `refcase_ws` is located next to `experiments` at `../catkin_ws`.
...
- This README is intended for running experiments from the `experiments` folder, not directly from `refcase_ws`.
- The state machine is launched from `refcase_ws`, but the orchestration script lives in `experiments`.
